        ...
        while self.playback_active:
            try:
                if not self.should_play_audio:
                    # If playback is paused, clear the queue and wait
                    try:
                        while not self.playback_queue.empty():
                            self.playback_queue.get_nowait()
                    except:
                ...
                # Check if output stream is valid, if not reinitialize
                ...
                try:
                    ...
                    if self.output_stream and len(audio_data) > 0:
                        # Make sure audio_data is bytes
    ...
    def on_open(self, ws):
        ...
        init_event = {
            ...
            "session": {
                ...
                "turn_detection": {
                    "type": "server_vad",
                    "threshold": 0.1,  # Adjusted back to a less sensitive threshold
            ...
            while self.conversation_active:
                try:
                    if not self.audio_manager.audio_queue.empty():
                        ...
                        # Always send audio data for processing
                        ...
                        time.sleep(0.01)  # Slightly longer sleep to prevent CPU overuse
                except Exception as e:
            ...
            if event_type == "session.created":
                ...
                if not self.is_speaking or time_since_last_vad > self.vad_threshold:
                    ...
                    if self.is_assistant_speaking or self.has_active_response:
                        ...
                        # First stop audio immediately before canceling the response
                        self.audio_manager.stop_audio()
                        # Then cancel the response
                ...
                if self.is_speaking:
                    ...
                    if transcribed_text:
                        ...
                        # Ensure we create a new response after sending the message
